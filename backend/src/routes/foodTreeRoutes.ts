...
export function createFoodTreeRoutes(foodTreeService: FoodTreeService): express.Router {
  ...
  router.get('/element/:id/nutrients', async (req, res) => {
    ...
    if (mass === null) {
      res.status(400).json({ error: 'invalid ?mass= parameter. expected positive number' });
      return;
    }
